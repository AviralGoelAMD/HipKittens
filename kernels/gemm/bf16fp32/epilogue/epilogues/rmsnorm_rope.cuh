...
#include "rotary.cuh"    // apply_rope_store
...
// B)), then the rotation runs register-local (r commutes with it). Stored in permuted column
// order -- valid because attention Q*K^T is invariant to a shared Q/K feature permutation.
...
struct RmsnormRopeEpilogue {
    ...
    static __device__ inline void apply(const Globals& g, Accum& C, int row,int col,int wr,int wc){
        ...
        apply_rope_store(g, C, row,col,wr,wc);   // rotate + store, one row sub-tile at a time
    }
};
