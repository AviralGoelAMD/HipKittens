/**
 * @file gemm_padded.cpp
 * @brief Rung 4 -- LDS-padded bf16 -> fp32 GEMM for gfx1250.
 *
 * Diff vs `gemm_async`: replace the flat LDS layout with `lds_pad_default`
 * (16 B padding every 256 B for bf16) and use `kittens::load_b128` for the
 * shared -> register transfer. The padding kills bank conflicts that
 * dominated the previous rung's LDS bandwidth.
 */
...
void gemm_padded_kernel(const gemm_globals g, int M, int N, int K)
{
...
void dispatch(gemm_globals g)
{
    ...
    hipFuncSetAttribute(reinterpret_cast<const void*>(gemm_padded_kernel),
                        hipFuncAttributeMaxDynamicSharedMemorySize, static_cast<int>(mem_size));
    gemm_padded_kernel<<<g.grid(), g.block(), mem_size, g.stream>>>(g, g.M(), g.N(), g.K());
}
