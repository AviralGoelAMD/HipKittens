...
void gemm_tdm_arrive_kernel(const gemm_globals g, int M, int N, int K)
{
    ...
    // model it natively.
    //
    // `async_barrier_arrive` internally guards with `laneid() == 0` so
    // each wave arrives exactly once per phase.
    if (wid == 0) {
        g2s::load_tdm<Pad, BLOCK_M, K_STEP>(
            A_lds[0], g.a, {0, 0, tile_m, 0}, M, K, K);
        sync::wait_tensor();
        sync::async_barrier_arrive(&A_bar[0].state);
    }
    if (wid == 1) {
        g2s::load_tdm<Pad, BLOCK_N, K_STEP>(
            B_lds[0], g.b, {0, 0, tile_n, 0}, N, K, K);
        sync::wait_tensor();
        sync::async_barrier_arrive(&B_bar[0].state);
    }
    ...
    for (int k = 0; k < k_iters; ++k) {
        ...
        if (k + 1 < k_iters) {
            if (wid == 0) {
                ...
                sync::wait_tensor();
                sync::async_barrier_arrive(&A_bar[nxt].state);
            }
            if (wid == 1) {
                g2s::load_tdm<Pad, BLOCK_N, K_STEP>(
                    B_lds[nxt], g.b, {0, 0, tile_n, k + 1}, N, K, K);
                sync::wait_tensor();
                sync::async_barrier_arrive(&B_bar[nxt].state);
            }
        }
